...
const workSheet = () => {
  ...
  req.onsuccess = function (evt) {
    ...
    var html =
      "<img src='images/btn-close@2x.png' style='cursor:pointer;margin-left:5px;width:50px;height:50px;' onclick=\"$('#worksheet').hide();$('#lpanel').hide();$('#panelout').show();$('#panelin').hide()\" ><label style='margin-left:20px;font:bold 30px Arial'>" +
      localStorage.getItem("pattern") +
      "</label><div style='text-align:left;font:Bold 30px Arial'></div><br/>";
    html +=
    ...
    for (var i = 0; i < window.holedata.length; i++) {
      if (window.holedata[i].blast === localStorage.getItem("pattern")) {
        var depth = (
